...
void do_pub(test_config_t *params) {
    ...
    data               = (struct test_data *)malloc(params->payload_size);
    ...
    while (g_running && (params->max_msg == 0 || counter < (params->max_msg))) {
        ...
        lunar_pub(params->topic, pub_cb, (void *)params->payload_size);
    }
...
size_t sub_cb(void *data, void *args) {
    struct test_msg *t = data;
    // No action required by this test
    return 0;
}
...
void do_sub(test_config_t *params) {
    ...
    while (g_running && (params->max_msg == 0 || counter < (params->max_msg))) {
        ...
        lunar_sub(params->topic, sub_cb, NULL);
        counter++;
    }
...
void do_pubsub(test_config_t *params) {
    ...
    while (g_running && (params->max_msg == 0 || counter < (params->max_msg))) {
        ...
        lunar_pub(params->topic, pubsub_ping_cb, (void *)data);

        // Receive Pong
        lunar_sub(params->topic, pubsub_pong_cb, (void *)data);
...
size_t subpub_pong_cb(void *data, void *arg) {
    struct test_data *t = (struct test_data *)data;
    memcpy(data, arg, t->payload_size);
    return t->payload_size;
}
void do_subpub(test_config_t *params) {
    ...
    while (g_running && (params->max_msg == 0 || counter < (params->max_msg))) {
        ...
        lunar_sub(params->topic, subpub_ping_cb, (void *)&data);
        LOG_TRACE("Forwarding sample\n");
        lunar_pub(params->topic, subpub_pong_cb, (void *)&data);
...
int parse_arguments(int argc, char *argv[], test_config_t *config) {
    ...
    printf("Running with the following arguments:   \n"
           ...
           dp_strings[config->dp_qos], config->topic, config->sleep_time);
