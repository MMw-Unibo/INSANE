...
fn client(local: SocketAddrV4, remote: SocketAddrV4) -> Result<()> {
    ...
    while npongs > 0 {
        ...
        let rt_time = Instant::now();

        // Push and get the token
        let qt_push: QToken = match libos.pushto2(sockfd, sendbuf.as_slice(), remote) {
            Ok(qt) => qt,
            Err(e) => panic!("failed to push: {:?}", e.cause),
        };
        ...
        // Check outcome
        let (_, result) = match libos.wait2(qt_push) {
            Ok(result) => result,
            // => {
            //     match result {
            //         (.., OperationResult::Push) => rt_time = Some(tmp),
            //         _ => {},
            //     }
            //     result
            // },
        ...
        // Wait to receive
        let (_, result) = match libos.wait2(qt_pop) {
            Ok(result) => {
                println!("{},{}", npings, rt_time.elapsed().as_nanos());
                npings += 1;
                npongs -= 1;
                result
            },
