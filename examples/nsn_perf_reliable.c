...
/*  !! WARNING !!
 *  The reliable feature is highly experimental yet. So, use with care.
 *  In particular, DO NOT SEND more than 1M messages in the throughuput test.
 *  If you want to do that, please modify the PENDING_POOL_SIZE macro in the proto_trp.c protocol.
 *  Please also note that other macros are defined there for the moment, and those can play a role
 * in how the test works. The latency test is still not implemented (it makes little sense...)
 */
...
void do_source(nsn_stream_t *stream, test_config_t *params) {
    ...
    while (g_running && (params->max_msg == 0 || counter < (params->max_msg))) {
        ...
        buf     = nsn_get_buffer_reliable(source, params->payload_size, 0);
        ...
        if (nsn_buffer_is_valid(&buf)) {
            ...
            ret = nsn_emit_data_reliable(source, buf);

            // LOG_DEBUG("%ld)\ttime: %ld (%lu)", counter, data->tx_time);
        }
    }
    printf("Finished sending %lu messages. Waiting for all acks...\n", counter);

    // TODO: This must be hidden somewhere else
...
void do_pong(nsn_stream_t *stream, test_config_t *params) {
    ...
    while (g_running) {
        buf_recv = nsn_consume_data(sink, 0);
        buf_send = nsn_get_buffer(source, buf_recv.len, 0);
